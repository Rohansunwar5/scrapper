...
async def retrieve_channel_names(search_query):
    try:
        modified_query1 = f'"{search_query}" AND "leak" OR "leaked" OR "hack" OR "hacked" OR "attack" OR "attacked" -telegraph -news'
        modified_query2 = f'"{search_query}" AND ("database leak" OR "data breach") AND ("hack" OR "database" OR "leak" OR "link" OR "download") -telegraph -news'
        # modified_query3 = f'"{search_query}" AND ("malware" OR "c2") AND ("hack" OR "trojan" OR "leak" OR "stealer") -telegraph -news'

        results1, results2 = await asyncio.gather(
            scrape_links(modified_query1), scrape_links(modified_query2)
        )

        all_links = results1 + results2 
